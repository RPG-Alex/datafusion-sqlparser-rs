...
impl fmt::Display for Whitespace {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            Whitespace::Comment(comment) => write!(f, "{}", comment),
        }
    }
}
                    ...
                    match chars.peek() {
                        Some('-') => {
                            ...
                            if is_comment {
                                ...
                                return Ok(Some(Token::Whitespace(Whitespace::Comment(
                ...
                '/' => {
                    ...
                    match chars.peek() {
                        ...
                        Some('/') if dialect_of!(self is SnowflakeDialect) => {
                            ...
                            let comment = self.tokenize_single_line_comment(chars);
                            Ok(Some(Token::Whitespace(Whitespace::Comment(
                                Comment::SingleLineComment {
                                    prefix: "//".to_owned(),
                                    comment,
                                },
        ...
        loop {
            match chars.next() {
                ...
                Some('*') if matches!(chars.peek(), Some('/')) => {
                    ...
                    nested -= 1;
                    if nested == 0 {
                        break Ok(Some(Token::Whitespace(Whitespace::Comment(
                            Comment::MultiLineComment(s),
                        ))));
                    }
                    s.push('*');
                    s.push('/');
